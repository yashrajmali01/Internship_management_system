...
if (isset($_SESSION['clgname'])) {
    $instituteCode = $_SESSION['clgname'];
    $status = "not approved";

    // Connect to the database
    $link = mysqli_connect("localhost:3306", "root", "", "intern") or die("failed to connect database");

    // Fetch data based on the institute code and optional location and role search
  ...
  <link href="lib/bootstrap/css/bootstrap.min.css" rel="stylesheet">
        ...
        .search-bar {
            display: flex;
            flex-direction: row; /* Display search options in a row */
...
<a href="index.html" class="navigation-bar__logo navbar-brand">
    ...
    <!-- Your existing PHP code for displaying data goes here -->
...
<div class="aj" style="background-color:azure">
        <div class="card-container" style="background-color: burlywood;">
            <?php
                ...
                while ($row = mysqli_fetch_assoc($result)) {
                    ...
                    <div class="card" id="job_<?php echo $jid; ?>">
                        ...
                        <form action='' method='post'>
                            ...
                            <button type='button' name='approve' onclick='approveJob("<?php echo $jid; ?>");'>Approve</button>
                        </form>
                    </div>
                    <?php
                }
    ...
    function approveJob(jobId) {
        ...
        if (isDateNotExpired(endDate)) {
            ...
            $.ajax({
                type: "POST",
                url: "approve_job.php", // Replace with the actual path to your PHP script
                data: {job_id: jobId},

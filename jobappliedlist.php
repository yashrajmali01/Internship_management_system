...
if ($result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) {
        ...
        // Button for approving job applications with confirmation
        echo "<button type='submit' class='btn btn-xs btn-success' name='approve' value='".$row['id']."' onclick='return CheckApprove()'>Approve</button>";
        
        // Button for rejecting job applications with confirmation
        echo "<button type='submit' class='btn btn-xs btn-danger' name='reject' value='".$row['id']."' onclick='return CheckReject()'>Reject</button>";
...
// Check if either Approve or Reject button is clicked
if(isset($_POST['approve'])) {
    $id = $_POST['approve'];
    // Approve kelyane karayla code
    $approve_query = "UPDATE jobapplied SET status='Approved' WHERE id=$id";
    $approve_result = mysqli_query($conn, $approve_query);
    if($approve_result) {
        // echo "<script>alert('Job application approved successfully!');</script>";
    } else {
        echo "<script>alert('Error approving job application!');</script>";
    }
} elseif(isset($_POST['reject'])) {
    $id = $_POST['reject'];
    // Reject kelyane karayla code
    $reject_query = "UPDATE jobapplied SET status='Rejected' WHERE id=$id";
    $reject_result = mysqli_query($conn, $reject_query);
    if($reject_result) {
         echo "<script>alert('Job application rejected successfully!');</script>";
    } else {
        echo "<script>alert('Error rejecting job application!');</script>";
    }
}
?>
...
<script>
    function CheckApprove() {
        return confirm('Are you sure you want to approve this job application?');
    }

    function CheckReject() {
        return confirm('Are you sure you want to reject this job application?');
    }
</script>
  ...
  <div class="container">
    ...
    <div class="container">
        <div class="row">
            ...
            <div class="col-md-3 mt-5">
                ...
                <ul class="list-unstyled mb-4">
                    ...
                    <li><a href="#">Internship in Delhi NCR</a></li>
        ...
        <div class="row">
            <div class="col-md-12 text-center">
                <p>&copy; 2024 - INTERNSHIPGATE, A Unit of ISAN DATA SYSTEMS</p>
            </div>
        </div>
    </div>
</footer> -->
